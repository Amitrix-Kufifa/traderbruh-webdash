...
# + Enriched playbook-style commentary (TA + Fundies + Patterns + Auto-DCA)
...
def fetch_fundamentals(symbol: str):
    """Fetch key fundamental stats via yfinance info and score them 0–5."""
    try:
        ...
        # 1. Quality (ROE)
        if data['roe'] > FUNDY_RULES['roe_min']:
            score += 1

        # 2. Profitability (Margins)
        if data['margins'] > FUNDY_RULES['margin_min']:
            score += 1

        # 3. Safety (Liquidity OR Net Cash)
        if data['curr_ratio'] > FUNDY_RULES['curr_ratio_min'] or data['cash'] > data['debt']:
            score += 1

        # 4. Growth (Revenue)
        if data['rev_growth'] > FUNDY_RULES['growth_min']:
            score += 1

        # 5. Value (PEG or PE)
        val_ok = False
        if data['peg'] and 0 < data['peg'] < FUNDY_RULES['peg_max']:
            val_ok = True
        if data['pe'] and 0 < data['pe'] < FUNDY_RULES['pe_max']:
            val_ok = True
        if val_ok:
            score += 1
        ...
        return {
            'roe': 0, 'margins': 0, 'curr_ratio': 0,
            'debt': 0, 'cash': 0, 'rev_growth': 0,
            'peg': 0, 'pe': 0, 'mcap': 0,
...
def parse_3y_stats(text: str):
    ...
    parts = [
        p for p in [
            act,
            f"{shares} shares" if shares else None,
            f"A${value}" if value else None
        ] if p
    ]
...
def read_pdf_first_text(path: str):
    if not HAVE_PYPDF:
        return ''
    try:
        return re.sub(r'[ \t]+', ' ', PdfReader(path).pages[0].extract_text() or '')
    except Exception:
        return ''
...
def parse_announcements():
    ...
    for fp in sorted(glob.glob(os.path.join(ANN_DIR, '*.pdf'))):
        ...
        _type, tag = next(
            ((l, t) for l, p, t in NEWS_TYPES if re.search(p, fname + " " + text, re.I)),
            ('Announcement', 'gen')
        )
        d = None
        if (md := re.search(r'(\d{1,2}\s+[A-Za-z]{3,9}\s+20\d{2})', text)):
            try:
                d = datetime.strptime(md.group(1), '%d %B %Y')
            except Exception:
                try:
                    d = datetime.strptime(md.group(1), '%d %b %Y')
                except Exception:
...
def is_euphoria(r: pd.Series) -> bool:
    """Extended, late-stage move near highs with stretched RSI."""
    return (
        (r['Dist_to_52W_High_%'] > -3.5) and
        (r['Dist_to_SMA200_%'] > 50.0) and
        (r['RSI14'] >= 70.0)
    )

def comment_for_row(r: pd.Series) -> str:
    """
    Combined technical + fundamental commentary for a snapshot row.
    This is what shows up in the main cards and Signals table.
    """
    ...
    tier = str(r.get('Fundy_Tier', 'Unknown') or 'Unknown')

    # Fundamentals tail: how to treat the idea differently based on business quality
    fundy_tail = ""
    if f_score <= 1 and sig in ('BUY', 'DCA', 'WATCH'):
        fundy_tail = (
            " ⚠️ Fundamentals are weak (low score), so treat this as a speculative/tactical trade and size down."
        )
    elif f_score >= 3 and sig in ('BUY', 'DCA'):
        fundy_tail = (
            " 💼 Underlying business quality is decent to strong, so this can support a core position if your thesis holds."
        )
    elif f_score >= 3 and sig == 'WATCH':
        fundy_tail = (
            " 💎 High-quality business on watch — worth prioritising if the technical trigger appears."
        )
    ...
    if sig == 'WATCH':
        if is_euphoria(r):
            base = (
                ...
                "Existing holders: consider banking partial profits or tightening risk (e.g. stops near the recent "
                "breakout area or key short-term moving averages). New entries are high-risk, late-trend trades — "
                "keep size small and define your invalidation before buying."
            )
            return base + fundy_tail

        if d52 > -2:
            dist_s = f"within {abs(d52):.1f}% of its 52-week high"
        elif d52 > -6:
            dist_s = f"only {abs(d52):.1f}% below the recent high zone"
        elif d200 > 0:
            dist_s = f"trading {d200:.1f}% above the 200DMA (uptrend still intact)"
        else:
            dist_s = f"{abs(d200):.1f}% below the 200DMA (trying to reclaim trend)"
        ...
        base = (
            f"{dist_s}, and {rsi_txt}. Price is near a potential inflection zone — watch for either a clean breakout "
            f"through recent highs or a healthier pullback/retest before committing fresh capital."
        )
        return base + fundy_tail

    base = (
        "Neutral setup — trend and momentum are not giving a strong edge yet; let more price action build the story "
        "before acting."
    )
    return base + fundy_tail
...
for t, y in UNIVERSE:
    ...
    bias = pattern_bias(pname)
    sig_lower = str(sig).lower()
    sig_side = 'neutral'
    if sig_lower in ('buy', 'dca'):
        sig_side = 'bullish'
    elif sig_lower == 'avoid':
        sig_side = 'bearish'
    palign = 'ALIGNED' if (bias == 'neutral' or sig_side == 'neutral' or bias == sig_side) else 'CONFLICT'
    ...
    snaps.append({
        ...
        'Fundy_PE': fundy['pe'],
        'Fundy_PEG': fundy['peg'],
        'Fundy_Growth': fundy['rev_growth'],
        'Fundy_Cash': fundy['cash'],
        'Fundy_Debt': fundy['debt']
    })
...
if not news_df.empty:
    news_df['Recent'] = news_df.apply(
        lambda r: (
            (today_syd - datetime.strptime(r['Date'], '%Y-%m-%d').date()).days <= NEWS_WINDOW_DAYS
            if r['Date'] else False
        ),
        axis=1
    )
...
def rank(df):
    # Prioritize higher fundamental score in ranking
    buy   = df[df.Signal == 'BUY'].sort_values(
        ['Fundy_Score', 'Dist_to_52W_High_%'],
        ascending=[False, False]
    )
    dca   = df[df.Signal == 'DCA'].sort_values(
        ['Fundy_Score', 'Dist_to_SMA200_%'],
        ascending=[False, True]
    )
    watch = df[df.Signal == 'WATCH'].sort_values(
        ['Fundy_Score', 'Dist_to_52W_High_%'],
        ascending=[False, False]
    )
    avoid = df[df.Signal == 'AVOID'].sort_values(
        'Fundy_Score',
        ascending=True
    )
...
NEWSCOUNT = len(news_df) if not news_df.empty else 0
...
for section, df, badge in [
    ...
    if not df.empty:
        ...
        html_cards.append(
            f"<h2 id='{badge}' style='margin-top:30px; font-size:18px; color:var(--text-muted)'>{section}</h2>"
            f"<div class='grid'>{grid_items}</div>"
        )

counts = snaps_df['Signal'].value_counts()
hq_count = int((snaps_df['Fundy_Score'] >= 3).sum()) if not snaps_df.empty else 0

kpi_html = f"""
<div class="kpi-scroll">
    {render_kpi('High Quality (Fundy ≥3)', hq_count, 'text-main')}
    ...
    <div class="container">
        <div style="margin-bottom:20px">
            ...
            <div style="color:var(--text-muted); font-size:13px">
                ASX Technical Analysis &amp; Fundamentals • Updated {datetime.now(SYD).strftime('%I:%M %p %Z')}
            </div>
        </div>
        ...
        <div class="card">
            <div class="playbook">
                ...
                1. <b>Quality:</b> ROE &gt; 15% (efficient use of equity).<br>
                2. <b>Profitability:</b> Net Margins &gt; 10% (pricing power / moat).<br>
                3. <b>Safety:</b> Current Ratio &gt; 1.2 or Cash &gt; Debt (can they pay the bills?).<br>
                4. <b>Growth:</b> Revenue Growth &gt; 10% (is the business actually expanding?).<br>
                5. <b>Value:</b> PEG &lt; 2.0 or P/E &lt; 25 (are you massively overpaying?).<br><br>
                <b>Use:</b> If a stock is a "BUY" technically but has a <b>1/5 Score</b>, it is a speculative trade —
                size down and use tight stops. Scores of <b>4–5/5</b> indicate stronger business quality that can support
                core positions when the chart and thesis agree.
            </div>
        ...
        <div class="card">
            <div class="playbook">
                <b>Playbook:</b> Gap-down <b>reaction</b> setups. Focus on names where the day-1 gap down was at least {abs(RULES['autodca']['gap_thresh']):.1f}%,
                the next close has reclaimed the prior day's midpoint, price is back above EMA21, and a decent portion of the gap has
                already been filled. Typical use: scale in 2–4 small adds into strength <i>after</i> the reclaim, with a stop near the
                gap low or recent swing low. Avoid names that are still failing the midpoint/EMA21 tests.
            </div>
            <div class="table-responsive">
                <table>
                    <thead>
        ...
        <div class="card">
            <div class="playbook">
                <b>Playbook:</b> High-conviction patterns in the last {PATTERN_LOOKBACK} days — use them as <b>context</b> for your plan,
                not fire-and-forget signals.<br>
                • <b>Ascending triangles / flat tops</b> favour breakouts above resistance; look for strong closes through the neckline with
                  volume, then use recent swing lows or the range as your risk line.<br>
                • <b>Double bottoms / inverse H&amp;S</b> favour reversals off support; better entries tend to be near the second low with clear
                  rejection, invalidated if price breaks the base.<br>
                • <b>Double tops / head &amp; shoulders / descending triangles</b> are topping or distribution structures; for longs, that usually
                  means tighten risk and stop adding rather than chase new highs.<br>
                <b>Alignment</b> shows whether the pattern bias (bullish/bearish/neutral) agrees with the current signal bucket
                (BUY/DCA/WATCH vs AVOID).
            </div>
            <div class="table-responsive">
                <table>
                    <thead>
        ...
        <div class="card" style="padding:0">
            <div class="table-responsive">
                <table>
                    ...
                    <tbody>{news_rows if news_rows else "<tr><td colspan='4' style='text-align:center; color:var(--text-muted)'>No announcements loaded.</td></tr>"}</tbody>
                </table>
            </div>
        </div>
