...
# Architecture: The "Bible" Logic x Multi-Market Tabs
# Features: Deep Fundamentals (0-10), Full Pattern Engine, Matrix Logic, News Parsing
...
# ---------------- Global Config ----------------
SYD = zoneinfo.ZoneInfo('Australia/Sydney') # Kept for local file timestamps
...
MARKETS = {
    'AU': {
        ...
        'tz': 'Australia/Sydney',
        'suffix': '.AX',
        'news': True, # Enable PDF parsing
        'stocks': {
            'CSL': ('CSL Limited', 'Biotech: vaccines, plasma.'),
            'COH': ('Cochlear', 'Implantable hearing devices.'),
    ...
    'IN': {
        ...
        'news': False,
        'stocks': {
            'RELIANCE': ('Reliance Ind', 'Conglomerate.'),
...
def fetch_prices(symbol: str, timezone: str) -> pd.DataFrame:
    """Fetch OHLCV history with Timezone Awareness."""
    try:
...
def fetch_deep_fundamentals(symbol: str):
    """Deep Dive: 0-10 Quality Score calculation."""
    try:
        ...
        marg_curr = info.get('profitMargins', 0) or 0
        ...
        return {
            ...
            'pe': pe, 'peg': peg, 'cash': cash
        }
    except Exception:
        return {'score': 0, 'tier': 'Error', 'roe_3y':0, 'margins':0, 'debt_eq':0, 'rev_cagr':0, 'is_buyback':False, 'fcf_yield':0, 'pe':0, 'peg':0, 'cash': 0}
...
def _expand_lines(lines):
    ...
    for ln in lines:
        if ln[0] == 'h': _, d1, d2, y = ln; out.append(('h', d1, d2, y))
...
def mini_candle(ind, flag_info=None, pattern_lines=None):
    ...
    if pattern_lines:
        for (kind, *args) in _expand_lines(pattern_lines):
            if kind == 'h':
                x1, x2, y = args
                fig.add_trace(go.Scatter(x=[x1, x2], y=[y, y], mode='lines', line=dict(width=2, color='rgba(34,197,94,0.95)', dash='dot'), hoverinfo='skip', showlegend=False))
            else:
                x1, y1, x2, y2 = args
                fig.add_trace(go.Scatter(x=[x1, x2], y=[y1, y2], mode='lines', line=dict(width=2, color='rgba(234,179,8,0.95)'), hoverinfo='skip', showlegend=False))
...
for mkt_code, config in MARKETS.items():
    ...
    for ticker, meta in config['stocks'].items():
        ...
        if dt_pats: breakout_ready, breakout_info = breakout_ready_dt(ind, dt_pats[0], BREAKOUT_RULES)
        ...
        r = {
            ...
            'Signal': sig, 'SignalAuto': bool(signal_auto),
            ...
            'Flag': bool(flag_flag), 
            'AutoDCA_Flag': bool(gate_flag), 'AutoDCA_Gap_%': float(gate_det.get('gap_pct', np.nan)),
        ...
        r['_mini_candle'] = mini_candle(ind, flag_det if flag_flag else None, r['_pattern_lines'])
        rows.append(r)
    
    df_mkt = pd.DataFrame(rows)
    if df_mkt.empty: continue

    # 4. Generate HTML Sections (Tabs)
    ...
    def render_card(r, badge_type):
        ...
        <div class="card searchable-item">
            <div class="card-header">
                ...
                <div class="price-block"><div class="price-main mono">{r['LastClose']:.2f}</div><div class="price-sub">{r['LastDate']}</div></div>
            </div>
            <div class="metrics-row">
                ...
                <div class="metric"><label>vs 52W</label><span class="mono">{r['Dist_to_52W_High_%']:+.1f}%</span></div>
            </div>
